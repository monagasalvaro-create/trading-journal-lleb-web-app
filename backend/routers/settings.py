...
async def get_or_create_settings(db: AsyncSession, account_id: str = "default") -> Settings:
    """Get existing settings for account_id or create with defaults. Never mutates ORM fields outside transactions."""
    result = await db.execute(select(Settings).where(Settings.id == account_id))
    settings = result.scalar_one_or_none()

    if not settings:
        settings = Settings(id=account_id, account_name="Account 1")
        db.add(settings)
        await db.commit()
